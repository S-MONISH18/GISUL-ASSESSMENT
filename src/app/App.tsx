...
type AppView = "dashboard" | "generate" | "review" | "stack";
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border backdrop-blur-xl bg-background/85">
        <div className="max-w-6xl mx-auto px-5 h-14 flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            {(["dashboard", "generate", "stack"] as const).map((v) => (
              <button
                key={v}
                onClick={() => navigate(v)}
              ...
              >
                {v === "dashboard" ? "My Sets" : v === "generate" ? "Generate" : "Tech Stack"}
              </button>
            ))}
          </div>
          ...
          {view === "stack" && (
            ...
            >
              ...
              <div className="mb-10">
                ...
                <div className="font-mono text-xs text-muted-foreground mb-2">BACKEND ARCHITECTURE</div>
                <h1 className="text-4xl font-black mb-3" style={{ fontFamily: "'Unbounded', sans-serif" }}>
                  How FlashMind<br />is built.
                </h1>
                <p className="text-muted-foreground max-w-xl text-sm leading-relaxed">
                  A decoupled SPA + REST API architecture. Every technology is chosen for production viability — not just demo use.
                </p>
              </div>
                ...
                {[
                  {
                    layer: "Frontend",
                    name: "Next.js 15",
                    sub: "App Router · Tailwind CSS",
                    tags: ["React 18", "TypeScript", "Vercel"],
                    icon: "⚡",
                    accent: "#60a5fa",
                  },
                  {
                    layer: "Backend",
                    name: "FastAPI",
                    sub: "Python 3.11 · Uvicorn",
                    tags: ["Pydantic v2", "JWT Auth", "bcrypt"],
                    icon: "🚀",
                    accent: "#34d399",
                  },
                  {
                    layer: "Database",
                    name: "MongoDB Atlas",
                    sub: "Motor async driver",
                    tags: ["Free Tier", "ObjectId refs", "Indexes"],
                    icon: "🍃",
                    accent: "#fb923c",
                  },
                  {
                    layer: "AI / ML",
                    name: "spaCy + T5",
                    sub: "HuggingFace Transformers",
                    tags: ["NLTK", "TF-IDF", "NER"],
                    icon: "🧠",
                    accent: "var(--primary)",
                  },
                ].map(({ layer, name, sub, tags, icon, accent }) => (
                  <div key={layer} className="rounded-2xl border border-border bg-card p-5 flex flex-col gap-3 hover:border-primary/30 transition-colors">
                    <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-auto pt-2 border-t border-border">
                      {tags.map((t) => (
                        <span key={t} className="font-mono text-[10px] px-2 py-0.5 rounded-full border"
                          style={{ color: accent, borderColor: `color-mix(in srgb, ${accent} 30%, transparent)`, backgroundColor: `color-mix(in srgb, ${accent} 8%, transparent)` }}>
                          {t}
                        </span>
                      ))}
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                {/* Left col: NLP Pipeline + Auth */}
                <div className="flex flex-col gap-8">

                  {/* NLP Pipeline */}
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "color-mix(in srgb, var(--primary) 12%, transparent)" }}>
                        <Cpu size={16} style={{ color: "var(--primary)" }} />
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted-foreground">nlp_service.py</div>
                        <h2 className="text-base font-bold">AI / NLP Pipeline</h2>
                      </div>
                    </div>
                    <div className="flex flex-col">
                      {[
                        { step: "Sentence Tokenization", tool: "NLTK", desc: "Raw notes split into sentences for downstream processing.", color: "#60a5fa" },
                        { step: "Named Entity Recognition", tool: "spaCy NER", desc: "Extracts PERSON, ORG, GPE, DATE entities per sentence.", color: "#a78bfa" },
                        { step: "Keyword Extraction", tool: "spaCy POS", desc: "Noun chunks (NN, NNS, NNP) identified as key terms.", color: "#fb923c" },
                        { step: "Sentence Ranking", tool: "TF-IDF", desc: "Top N sentences scored and selected by TF-IDF weight sum.", color: "#f472b6" },
                        { step: "Q&A Generation", tool: "T5-small-qg", desc: "Each top sentence fed into valhalla/t5-small-qg-prepend to produce a question.", color: "var(--accent)" },
                        { step: "Deduplication", tool: "difflib", desc: "Near-duplicate pairs removed via SequenceMatcher (threshold > 0.85).", color: "#34d399" },
                        { step: "Title Generation", tool: "spaCy", desc: "Most frequent noun phrase across all sentences becomes the set title.", color: "#fbbf24" },
                      ].map(({ step, tool, desc, color }, i, arr) => (
                        <div key={step} className="flex gap-4">
                          <div className="flex flex-col items-center">
                            <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-mono font-bold flex-shrink-0"
                              style={{ backgroundColor: `color-mix(in srgb, ${color} 14%, transparent)`, border: `1px solid color-mix(in srgb, ${color} 30%, transparent)`, color }}>
                              {i + 1}
                            </div>
                            {i < arr.length - 1 && <div className="w-px flex-1 my-1" style={{ backgroundColor: "var(--border)" }} />}
                          </div>
                          <div className="pb-5 flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-0.5">
                              <span className="text-sm font-semibold text-foreground">{step}</span>
                              <span className="font-mono text-[10px] px-2 py-0.5 rounded-full border"
                                style={{ color, borderColor: `color-mix(in srgb, ${color} 30%, transparent)`, backgroundColor: `color-mix(in srgb, ${color} 8%, transparent)` }}>
                                {tool}
                              </span>
                            </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "color-mix(in srgb, #34d399 12%, transparent)" }}>
                        <Lock size={16} style={{ color: "#34d399" }} />
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted-foreground">auth_service.py</div>
                        <h2 className="text-base font-bold">Authentication Flow</h2>
                      </div>
                    </div>
                    <div className="flex flex-col gap-3">
                      {[
                        { label: "Signup", detail: "POST /auth/signup → bcrypt hash (rounds=12) → store user → 201", color: "#34d399" },
                        { label: "Login", detail: "POST /auth/login → verify hash → issue JWT (HS256, 24h expiry)", color: "#60a5fa" },
                        { label: "Protected Routes", detail: "FastAPI Depends(get_current_user) on all /flashcards/* endpoints", color: "#a78bfa" },
                        { label: "Frontend", detail: "Axios interceptor attaches Authorization: Bearer <token> to every request", color: "#fb923c" },
                      ].map(({ label, detail, color }) => (
                        <div key={label} className="flex gap-3 p-4 rounded-xl border border-border bg-card">
                          ...
                          <div>
                            ...
                            <code className="font-mono text-[11px] text-muted-foreground leading-relaxed">{detail}</code>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* Right col: API Endpoints + Spaced Repetition + Deployment */}
                <div className="flex flex-col gap-8">

                  {/* REST API */}
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "color-mix(in srgb, #fb923c 12%, transparent)" }}>
                        <GitBranch size={16} style={{ color: "#fb923c" }} />
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted-foreground">prefix: /api/v1</div>
                        <h2 className="text-base font-bold">REST API Endpoints</h2>
                      </div>
                    </div>
                    <div className="rounded-2xl border border-border bg-card overflow-hidden">
                      {[
                        { method: "POST", path: "/auth/signup", desc: "Register · bcrypt hash", methodColor: "#34d399" },
                        { method: "POST", path: "/auth/login", desc: "Login · issue JWT 24h", methodColor: "#34d399" },
                        { method: "POST", path: "/flashcards/generate", desc: "Run NLP pipeline · save set", methodColor: "#34d399" },
                        { method: "GET",  path: "/flashcards/sets", desc: "All sets for current user", methodColor: "#60a5fa" },
                        { method: "GET",  path: "/flashcards/sets/{id}", desc: "Cards ordered by review_weight↓", methodColor: "#60a5fa" },
                        { method: "PATCH", path: "/flashcards/{id}/review", desc: "Update SM-2 weight", methodColor: "#fb923c" },
                      ].map(({ method, path, desc, methodColor }) => (
                        <div key={path} className="flex items-center gap-3 px-4 py-3 border-b border-border last:border-0 hover:bg-secondary/40 transition-colors">
                          <span className="font-mono text-[10px] px-2 py-0.5 rounded font-bold w-12 text-center flex-shrink-0"
                            style={{ color: methodColor, backgroundColor: `color-mix(in srgb, ${methodColor} 10%, transparent)` }}>
                            {method}
                          </span>
                          <code className="font-mono text-xs text-foreground flex-1 truncate">{path}</code>
                          <span className="text-[11px] text-muted-foreground hidden sm:block flex-shrink-0">{desc}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "color-mix(in srgb, var(--accent) 12%, transparent)" }}>
                        <BarChart3 size={16} style={{ color: "var(--accent)" }} />
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted-foreground">Simplified SM-2</div>
                        <h2 className="text-base font-bold">Review Algorithm</h2>
                      </div>
                    </div>
                    <div className="rounded-2xl border border-border bg-card p-5 flex flex-col gap-4">
                      <div className="grid grid-cols-3 gap-3 text-center text-xs pb-4 border-b border-border">
                        <div>
                          <div className="font-mono text-[10px] text-muted-foreground mb-1">Initial Weight</div>
                          <div className="font-bold text-foreground" style={{ fontFamily: "'Unbounded', sans-serif" }}>1.0</div>
                        </div>
                        <div>
                          <div className="font-mono text-[10px] text-muted-foreground mb-1">Max Weight</div>
                          <div className="font-bold text-red-400" style={{ fontFamily: "'Unbounded', sans-serif" }}>3.0</div>
                        </div>
                        <div>
                          <div className="font-mono text-[10px] text-muted-foreground mb-1">Min Weight</div>
                          <div className="font-bold" style={{ fontFamily: "'Unbounded', sans-serif", color: "var(--accent)" }}>0.5</div>
                        </div>
                      </div>
                      <div className="flex flex-col gap-2.5">
                        <div className="flex items-start gap-3 p-3 rounded-xl border border-green-500/15 bg-green-500/5">
                          <CheckCircle size={14} className="text-green-400 mt-0.5 flex-shrink-0" />
                          <div>
                            <code className="font-mono text-xs text-green-400">Got It → weight = max(0.5, weight − 0.2)</code>
                            <p className="text-[11px] text-muted-foreground mt-1">Card surfaces less frequently in future sessions</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3 p-3 rounded-xl border border-red-500/15 bg-red-500/5">
                          <XCircle size={14} className="text-red-400 mt-0.5 flex-shrink-0" />
                          <div>
                            <code className="font-mono text-xs text-red-400">Still Learning → weight = min(3.0, weight + 0.5)</code>
                            <p className="text-[11px] text-muted-foreground mt-1">Card prioritised — appears first in next review session</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "color-mix(in srgb, #60a5fa 12%, transparent)" }}>
                        <Layers size={16} style={{ color: "#60a5fa" }} />
                      </div>
                      <div>
                        <div className="font-mono text-[10px] text-muted-foreground">Production</div>
                        <h2 className="text-base font-bold">Deployment</h2>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { platform: "Vercel", target: "Frontend", cmd: "next build", note: "Auto-deploy on git push", color: "#e2e8f0" },
                        { platform: "Render", target: "Backend API", cmd: "uvicorn main:app", note: "Free tier · ~50s cold start", color: "#a78bfa" },
                        { platform: "MongoDB Atlas", target: "Database", cmd: "Free Tier M0", note: "Motor async driver", color: "#34d399" },
                        { platform: "HuggingFace", target: "ML Models", cmd: "Loaded at startup", note: "@app.on_event('startup')", color: "#fbbf24" },
                      ].map(({ platform, target, cmd, note, color }) => (
                        <div key={platform} className="rounded-xl border border-border bg-card p-4">
                          <div className="font-mono text-[10px] text-muted-foreground mb-1">{target}</div>
                          <div className="font-semibold text-sm text-foreground mb-1">{platform}</div>
                          <code className="font-mono text-[10px] block mb-1" style={{ color }}>{cmd}</code>
                  ...
                  {/* Requirements */}
                  <div>
                    <div className="font-mono text-[10px] text-muted-foreground mb-3 uppercase tracking-widest">requirements.txt</div>
                    <div className="rounded-2xl border border-border bg-card p-4 grid grid-cols-2 gap-x-6 gap-y-1.5">
                      {[
                        "fastapi==0.111.0", "uvicorn==0.29.0",
                        "pymongo==4.7.2", "motor==3.4.0",
                        "python-jose==3.3.0", "bcrypt==4.1.3",
                        "spacy==3.7.4", "nltk==3.8.1",
                        "scikit-learn==1.4.2", "transformers==4.40.2",
                        "torch==2.3.0", "pydantic==2.7.1",
                      ].map((pkg) => {
                        ...
                        return (
                          <div key={pkg} className="flex items-center justify-between gap-2">
                            ...
                            <code className="font-mono text-[10px] text-muted-foreground">{ver}</code>
                          </div>
                        );
                      })}
                    </div>
